...
function compileScriptsBabel() {
    return HELPERS.execute('node --max-old-space-size=8192 ./node_modules/rollup/dist/bin/rollup -c rollup.config.mjs --configBabel');
}

function compileMjsScripts() {
    return HELPERS.execute('npx rollup -c rollup.mjs.config.mjs');
}
...
function mjsMin() {
    return gulp.src(['dist/raw-mjs/*.js', 'dist/raw-mjs/cultures/*.js', 'dist/raw-mjs/messages/*.js'], { base: "dist/raw-mjs" })
        .pipe(gulpIf(makeSourceMaps, sourcemaps.init()))
        .pipe(terser(terserOptions))
...
gulp.task('scripts', gulp.parallel(gulp.series(compileMjsScripts, compileScripts, minScripts)));
gulp.task('scripts:babel', gulp.parallel(gulp.series(compileMjsScripts, compileScriptsBabel, minScripts)));
gulp.task('scripts:mjs', gulp.parallel(gulp.series(compileMjsScripts, mjsMin)));

function minScripts() {
    return gulp.src(['dist/raw-js/*.js', 'dist/raw-js/cultures/*.js', 'dist/raw-js/messages/*.js'], { base: "dist/raw-js" })
